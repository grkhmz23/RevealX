...
export class CasinoEngine {
  private config: CasinoConfig = {
    ...
    minPoolReserve: 25, // Aligned with unified ticket costs for investor demo
...
export function generateGameSymbols(): string[] {
  return casinoEngine.calculateWin(0.1, 1).symbols;
}

export function checkWin(symbols: string[]): { isWin: boolean; multiplier: number } {
  const allMatch = symbols.every(symbol => symbol === symbols[0] && symbol !== '❌');
  return {
    isWin: allMatch,
    multiplier: allMatch ? multipliers[Math.floor(Math.random() * multipliers.length)] : 0
  };
}
