...
import { casinoEngine } from '@/lib/casino-logic';
...
function PoolInfo({ ticketCost }: { ticketCost: number }) {
  ...
  return (
    <div className="space-y-1">
      <div>Max Win: {formatAmount(winRateInfo.maxPayout)} • Win Rate: {(winRateInfo.currentRate * 100).toFixed(1)}%</div>
      <div className={`text-xs ${isHealthy ? 'text-green-400' : 'text-yellow-400'}`}>
        Pool: {formatAmount(poolBalance)} • House Edge: {(casinoEngine.getHouseEdge() * 100).toFixed(0)}%
      </div>
    </div>
  );
};
...
export function ScratchCardModal({
  ...
  const { isSolana, isBase, nativeCurrency } = useChain();
  const { formatAmount } = useChainConfig();
  ...
  // Base wallet hooks for real mode
  const { address: baseAddress, isConnected: baseConnected } = useAccount();
  const baseChainId = useChainId();
  const { purchase: purchaseBase, isWritePending: isBasePurchasePending } = useBasePurchase();
        ...
        if (isSolana) {
          ...
          try {
            ...
            toast({
              ...
              description: `Successfully paid ${formatAmount(ticketCost)}. Verifying on-chain...`,
            ...
            if (errorMsg.toLowerCase().includes('insufficient')) {
              userFriendlyMsg = `Insufficient ${nativeCurrency} balance. Please add funds to your wallet.`;
          ...
          try {
            ...
            toast({
              title: "Game Started",
              description: `Spent ${formatAmount(ticketCost)}. Scratch each zone to reveal!`,
          ...
          if (!baseConnected || !baseAddress) {
            toast({
              title: "Wallet Required",
              description: "Please connect your Base wallet to play in Real Mode",
              variant: "destructive",
            });
            onClose();
            return;
          ...
          const poolWallet = import.meta.env.VITE_BASE_POOL_WALLET as `0x${string}` | undefined;
          if (!poolWallet) {
            toast({
              title: "Config Error",
              description: "Base pool wallet not configured. Please contact support.",
              variant: "destructive",
            });
            onClose();
            return;
          ...
          setTransactionPending(true);
          try {
            const hash = await purchaseBase({
              ticketCost,
              poolWallet,
              chainId: baseChainId,
            });
            purchaseSignature = hash;
  ...
  const handleGameComplete = async () => {
    ...
    if (!isDemoMode) {
      const playerWallet = isSolana ? wallet.publicKey?.toString() : baseAddress;
      if (playerWallet) {
    ...
    if (finalOutcome.canWin && !isDemoMode) {
      ...
      console.log(`🏆 Win amount: ${winAmount} ${nativeCurrency}`);
      console.log(`🏆 Player wallet: ${playerWallet}`);
      console.log(`🏆 Ticket cost: ${ticketCost} ${nativeCurrency}`);
      console.log(`🏆 Multiplier: ${finalOutcome.multiplier}x`);

      if (!playerWallet || !createdGameId) {
        console.error('Missing player wallet or game ID for payout');
        return;
      }
      ...
      try {
        ...
        toast({
          title: "🎉 You Won!",
          description: `${formatAmount(winAmount)} has been sent to your wallet!`,
          className: "bg-green-600/20 border-green-600/50",
        });
      ...
      toast({
        title: "🎉 Demo Win!",
        description: `You would have won ${formatAmount(winAmount)} in Real Mode!`,
        className: "bg-neon-orange/20 border-neon-orange/50",
      });
    }
  ...
  const isPaymentProcessing = transactionPending || isBasePurchasePending;
          ...
          {(loading || isPaymentProcessing) ? (
            <div className="text-center py-12">
              <div className={`${cardDesign.accentColor} text-lg font-bold mb-4`}>
                {isPaymentProcessing ? "Processing Payment..." : "Initializing Game..."}
              </div>
              ...
              {gameResult.isWin && (
                <>
                  <div className="text-4xl font-black text-electric-blue mb-2">
                    {formatAmount(gameResult.winAmount)}
                  </div>
